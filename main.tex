            ...
            \footnotesize
              ...
              \item[] \faDatabase \quad \mycode{pickle} was originally designed for on-disk
               ...
               \item[]<2-> \faExchange \quad Now, it is used wildely to communicate objects between,
                   process, which is done in-memory. \textbf{RAM usage becomes
                       a critical concern.}
        \end{itemize}
        \vspace{1em}
        \uncover<3->{
...
\begin{frame}[fragile]{out-of-band serialization}
  ...
  \node[draw, rectangle,             align=center, minimum height=0.5cm] (1)
      {\footnotesize shape};
  \node[draw, rectangle, right=0.1cm of 1, align=center, minimum height=0.5cm] (2)
      {\footnotesize shape};
  \node[draw, rectangle, right=0.1cm of 2, align=center, minimum height=0.5cm] (3)
      {\footnotesize flags};
  ...
  \node[align=left, fill=gray!10] (legend) at (nparray.south) {numpy array};
  \node[visible on =<2->,draw=olive, thick, inner sep=0.1cm, fit=(1) (1) (1) (3), rectangle]  (metadata){};
  \node (picklestream) at (6, 2) {};
...
\begin{frame}[fragile]{Conclusion}
        ...
        which backend (processes vs. threads) to use is a
        ...
        \begin{tabular}{m{0.5cm} m{10cm}}
            \includegraphics[width=\linewidth] {media/green-tick-emoji.png} &
        thankfully, activating parallelism, and switching backends can be
            done using one line of code in \mycode{joblib}
        \end{tabular}
        }
\end{frame}
\end{document}
